...
const CarDetail = () => {
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      {/* FULL-WIDTH CAR IMAGE с FOMO - как в ТЗ */}
      <div className="relative w-full h-[60vh] min-h-[500px] bg-gray-900">
        ...
        {/* FOMO Overlay Elements */}
        <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent">
          
          {/* Top Left - Deal ID & Verified */}
          <div className="absolute top-6 left-6 flex flex-col gap-2">
            ...
            <div className="bg-black/70 backdrop-blur-sm text-white px-3 py-1.5 rounded text-sm">
              Deal ID: #{carData.id?.slice(-6).toUpperCase()}
            </div>
          </div>
          
          {/* Top Right - Timer ОГРОМНЫЙ */}
          <div className="absolute top-6 right-6 bg-red-600 text-white rounded-xl p-4 shadow-2xl">
            <div className="text-xs uppercase mb-1">DEAL ENDS IN</div>
            <div className="text-3xl font-bold">
              {timeRemaining.days}d {timeRemaining.hours}h {timeRemaining.minutes}m
            </div>
          </div>
          
          {/* Bottom - Title & Key Info */}
          <div className="absolute bottom-6 left-6 right-6">
            <h1 className="text-4xl font-bold text-white mb-3">{carData.title}</h1>
            
            <div className="flex flex-wrap gap-4 text-white">
              <div className="bg-white/20 backdrop-blur-sm px-4 py-2 rounded-lg">
                <div className="text-xs">MSRP</div>
                <div className="text-xl font-bold line-through">${carData.msrp?.toLocaleString()}</div>
              </div>
              
              <div className="bg-green-600 px-6 py-2 rounded-lg shadow-xl">
                <div className="text-xs">YOU SAVE</div>
                <div className="text-2xl font-bold">${carData.savings?.toLocaleString()}</div>
              </div>
              
              <div className="bg-white text-gray-900 px-6 py-2 rounded-lg shadow-xl">
                <div className="text-xs text-gray-600">YOUR PRICE</div>
                <div className="text-2xl font-bold">${carData.fleet?.toLocaleString()}</div>
              </div>
              
              <div className="bg-orange-600 px-4 py-2 rounded-lg">
                <div className="text-xs">DISCOUNT</div>
                <div className="text-xl font-bold">{((carData.savings / carData.msrp) * 100).toFixed(1)}% OFF</div>
              </div>
            </div>
          </div>
        </div>
