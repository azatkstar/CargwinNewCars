...
## 📡 Backend URL
```
https://lexus-preview.preview.emergentagent.com/api
```
...
```bash
curl -X POST "https://lexus-preview.preview.emergentagent.com/api/auth/register" \
...
```bash
curl -X PUT "https://lexus-preview.preview.emergentagent.com/api/user/profile" \
  -H "Authorization: Bearer YOUR_ACCESS_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
    "credit_score": 720,
...
```bash
curl -X GET "https://lexus-preview.preview.emergentagent.com/api/cars" | jq '.[0].id'
```

Затем создайте заявку:
```bash
curl -X POST "https://lexus-preview.preview.emergentagent.com/api/applications?lot_id=2024-lexus-rx350-premium" \
  -H "Authorization: Bearer YOUR_ACCESS_TOKEN"
```
...
```bash
curl -X GET "https://lexus-preview.preview.emergentagent.com/api/applications" \
  -H "Authorization: Bearer YOUR_ACCESS_TOKEN"
```
...
```bash
curl -X PATCH "https://lexus-preview.preview.emergentagent.com/api/admin/users/user-test-001/role?role=editor" \
  -H "Authorization: Bearer ADMIN_ACCESS_TOKEN"
```
...
```bash
curl -X GET "https://lexus-preview.preview.emergentagent.com/api/admin/applications" \
  -H "Authorization: Bearer ADMIN_ACCESS_TOKEN"
```
...
```bash
curl -X PATCH "https://lexus-preview.preview.emergentagent.com/api/admin/applications/APP_ID/status?status=approved&admin_notes=Customer approved for financing" \
  -H "Authorization: Bearer ADMIN_ACCESS_TOKEN"
```
...
Перейдите по ссылке:
```
https://auth.emergentagent.com/?redirect=https://lexus-preview.preview.emergentagent.com/dashboard
```

### Шаг 2: После авторизации Google
Вы будете перенаправлены на:
```
https://lexus-preview.preview.emergentagent.com/dashboard#session_id=XXXXX
```

### Шаг 3: Обработать session_id на frontend
Frontend должен отправить:
```bash
curl -X POST "https://lexus-preview.preview.emergentagent.com/api/auth/oauth/session" \
  -H "Content-Type: application/json" \
  -d '{"session_id": "XXXXX"}'
```
...
## 🔓 Выход
```bash
curl -X POST "https://lexus-preview.preview.emergentagent.com/api/auth/logout" \
  -H "Authorization: Bearer YOUR_ACCESS_TOKEN"
```
...
```bash
curl -X GET "https://lexus-preview.preview.emergentagent.com/api/cars" | jq
```

### Проверить здоровье сервера
```bash
curl -X GET "https://lexus-preview.preview.emergentagent.com/api/health"
```
...
### 1. Войти как админ
```bash
TOKEN=$(curl -s -X POST "https://lexus-preview.preview.emergentagent.com/api/auth/login" \
  -H "Content-Type: application/json" \
